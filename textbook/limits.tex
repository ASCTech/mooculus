...
\begin{exercise} 
Use a table and a calculator to estimate $\lim_{x\to 0} {\sin
  x\over x}$.
\end{exercise}
...
\begin{exercise} 
Sketch a plot of $f(x) = \dfrac{x}{|x|}$ and explain why $
...
$f(g(x))=\sqrt{x^2+5}$ and $g(f(x))=(\sqrt{x})^2+5=x+5$.  Here is a
companion to theorem~\xrefn{thm:properties of limits} for composition:

\begin{theorem} Suppose that $\lim_{x\to a}g(x)=L$ and $\lim_{x\to L}f(x)=f(L)$. Then
...
\begin{itemize}
\item $\lim_{x\to a} kf(x) = k\lim_{x\to a}f(x)=kL$ 
\item $\lim_{x\to a} (f(x)+g(x)) = \lim_{x\to a}f(x)+\lim_{x\to a}g(x)=L+M$  
\item $\lim_{x\to a} (f(x)g(x)) = \lim_{x\to a}f(x)\cdot\lim_{x\to a}g(x)=LM$ 
\item $\lim_{x\to a} \frac{f(x)}{g(x)} = \frac{\lim_{x\to a}f(x)}{\lim_{x\to
    a}g(x)}=\frac{L}{M},\text{ if $M\ne0$}$
\item $\lim_{x\to a} f(x)^n = \left(\lim_{x\to a}f(x)\right)^n=L^n$
\item $\lim_{x\to a} \sqrt[n]{f(x)} = \sqrt[n]{\lim_{x\to
...
&=\dfrac{\lim_{x\to 1}x^2-\lim_{x\to1}3x+\lim_{x\to1}5}{\lim_{x\to1}x-\lim_{x\to1}2} \\
...
\end{align*}
Hence $\lim_{x\to 3+} f(x) = -\infty$, $\lim_{x\to 3-} f(x) =
