...
\begin{tikzpicture}
	\begin{axis}[
	  ...
	  \addplot [very thick, penColor, smooth, domain=(4:10)] {3 + sin(deg(x*2))/(x-1)};
        ...
        \end{axis}
\end{tikzpicture}
\caption{A plot of a function with discontinuties at $x=4$ and $x=6$.}
\label{plot:discontinuous-function}
\end{marginfigure}

\begin{example}
Find the discontinuties (the values for $x$ where a function is not
...
Building from the definition of \textit{continous at a point}, we can
...
\end{cases}
\]
see Figure~\ref{plot:sqrt[5]{x}sin 1/x}. 
\end{example}
...
\begin{tikzpicture}
	\begin{axis}[
	  ...
	  \addplot [color=penColor, fill=penColor, very thick, smooth,domain=(-.02:.02)] {abs(x)^(1/5)} \closedcycle;
        ...
        \end{axis}
\end{tikzpicture}
\caption{A plot of $f(x)=\protect\sqrt[5]{x}\sin\left(\frac{1}{x}\right)$.}
