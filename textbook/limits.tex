...
\begin{exercise} 
Use a calculator to estimate $\ds\lim_{x\to 0} \frac{2^x-1}{x}$.
\end{exercise}
...
\begin{mainTheorem}[Limit Laws] Suppose that $\ds \lim_{x\to a}f(x)=L$, $\ds \lim_{x\to a}g(x)=M$, 
$k$ is some constant, and $n$ is a positive integer. Then
\begin{itemize}
