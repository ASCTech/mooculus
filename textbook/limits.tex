...
time, we'll assume that $x$ is positive---examine the diagrams below:
\begin{fullwidth}
\begin{tabular}{ccc}
\begin{tikzpicture}
	\begin{axis}[
...
Theorem~\ref{theorem:squeeze}, $\ds\lim_{x \to 0}\frac{\sin(x)}{x} = 1$.
\end{solution}
...
f(x) = \frac{1}{(x+1)^2}
\]
While the $\ds \lim_{x\to -1} f(x)$ does not exist, see
...
f(x) = \frac{1}{(x-1)}
\]
While we have $\ds \lim_{x\to 1} f(x) \ne \pm\infty$, we do have
one-sided limits, $\ds \lim_{x\to 1+} f(x) = \infty$ and $\ds
\lim_{x\to 1-} f(x) = -\infty$, see Figure~\ref{plot:1/(x-1)}.
\begin{marginfigure}
\begin{tikzpicture}
	\begin{axis}[
            domain=-1:2,
...
If $f(x)$ becomes arbitrarly close to a specific value $L$ by making
...
If $f(x)$ becomes
arbitrarly close to a specific value $L$ by making $x$ sufficiently
...
\lim_{x\to a} f(x) = \pm\infty, \qquad \lim_{x\to a+} f(x) = \pm\infty, \qquad\text{or}\qquad \lim_{x\to a-} f(x) = \pm\infty,
\]
then the line $x=a$ is a \textbf{horizontal asymptote} of $f(x)$.
\end{definition}
...
\end{example}
\begin{solution}
From Figure~\ref{plot:discontinuous-function} we see that $\ds \lim_{x\to 4} f(x)$ does not exist as
\[
\lim_{x\to 4-}f(x) = 1\qquad\text{and}\qquad \lim_{x\to 4+}f(x) \approx 3.5
\]
Hence $\ds \lim_{x\to 4} f(x) \ne f(4)$, and so $f(x)$ is not
continuous at $x=4$.

We also see that $\ds \lim_{x\to 6} f(x) \approx 3$ while $f(6) =
2$. Hence $\ds \lim_{x\to 6} f(x) \ne f(6)$, and so $f(x)$ is not
continuous at $x=6$.
\end{solution}
...
This function is continuous. Here we see how the informal definition
of continuity being that you can ``draw it'' without ``lifting your
pencil'' differs from the formal definition. 
\end{solution}
