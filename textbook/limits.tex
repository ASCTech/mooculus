...
\begin{tikzpicture}
	\begin{axis}[
            ...
            xtick={-2,...,4},
            ytick={-3,...,3},
            xlabel=$x$, xlabel style={at={(1,.5)}, anchor=west},
            ylabel=$y$, ylabel style={at={(.3,1)}, anchor=south}
          ]
	  \addplot [very thick, red!70!black, smooth] {x-1};
          \addplot[color=red!70!black,fill=white,only marks,mark=*] coordinates{(2,1)}; 
        \end{axis}
\end{tikzpicture}
...
< |x-a| < \delta$, $|f(x)-L|<\epsilon$. If no such value of $L$ can be
...
\begin{tikzpicture}
	\begin{axis}[
          ...
          \addplot [fill=black!10, smooth, domain=(0:1.570)] {5} \closedcycle;
	  ...
	  \addplot [fill=white, smooth,domain=(0:3)] {x*(x-2)^2+3*x} \closedcycle;
          \addplot [fill=black!10, draw=none, domain=.7:1.3] {x*(x-2)^2+3*x} \closedcycle;
	  ...
	  \addplot [very thick,red!70!black, smooth] {x*(x-2)^2+3*x};
        \end{axis}
\end{tikzpicture}
\caption{A geometric interpertation of the $\epsilon-\delta$ definition of a limit.}
