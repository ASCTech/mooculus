...
tangent line. For now, we will use the following informal definition:
...
contains the essence of the formal definition. We illustrate this
informal definition with Figure~\ref{figure:informal-tangent}.
\begin{figure*}
\begin{tikzpicture}
	\begin{axis}[
          ...
          \addplot[very thick,penColor, smooth,domain=(2.834:4.166)] {3.333/(2.225-.3*x)+0.163}; %% 2.5 to 4.333
          \addplot[very thick,penColor, smooth,domain=(5.334:6.666)] {11.11/(2.293-.09*x)-3.339}; %% 5 to 6.833
          \addplot[color=penColor,fill=penColor,only marks,mark=*] coordinates{(1,1)};  %% point to be zoomed

          \addplot [->,textColor] plot coordinates {(0,0) (0,6)}; %% make longer later axis
          \addplot [->,textColor] plot coordinates {(0,0) (2,0)}; %% make longer later axis
