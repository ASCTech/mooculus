...
\begin{theorem} For all $x$ in $\R$, $\cosh ^2 x -\sinh ^2 x = 1$.
...
\draw[->] (0,0) -- (3,0) ;
\draw[->] (0,-3) -- (0,3) ;
\foreach \x in {1,2,3} \draw (\x,0) -- (\x,-2pt) node[anchor=north] {\eightpoint
 $\x$};
...
\begin{exercise} Sketch the graphs of all six hyperbolic functions.
  You will likely find the discussion following 
theorem~\xrefn{thm:hyperbolic derivatives} helpful. 
...
of the hyperbolic functions. Refer to figure~\xrefn{fig:geometric def trigh}.


\begin{exercise} Use exercises~\xrefn{exer:sinhid} and~\xrefn{exer:coshid} to
show that $\sinh(2x)=2\sinh x \cosh x$ and $\cosh(2x)=\cosh^2 x
+\sinh^2 x$ for every $x$.  Conclude that $(\cosh (2x) -1)/2 = \sinh
^2 x$ $x$.

\begin{exercise}
Compute $\int \sqrt{x^2 -1}\,dx $. (Hint: make the substitution
...
$y=\tanh t$, $y=-\tanh t$,  and $y^2-x^2 =1$. Note well: $t$ is fixed,
