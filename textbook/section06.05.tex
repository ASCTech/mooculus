...
\exercise Let $\ds f(x) = x^2$.
Find a value $c\in (-1,2)$ so that $f'(c)$ equals the slope between
the endpoints of $f(x)$ on $[-1,2]$.
\begin{answer} $c=1/2$
\end{answer}

\exercise Verify that $f(x) = x/(x+2)$ satisfies the hypotheses of the
...
\exercise Let $f(x) = \tan x $. Show that $f(\pi ) = f(2\pi)=0$ but
there is no number $c\in (\pi,2\pi)$ such that $f'(c) =0$. Why does
this not contradict Rolle's theorem?

\exercise Let $\ds f(x) = (x-3)^{-2}$.  Show that there is no value 
$c\in (1,4)$ such that $f'(c) = (f(4)-f(1))/(4-1)$.  Why is
this not a contradiction of the Mean Value Theorem?

\exercise Describe all functions with derivative $\ds x^2+47x-5$.
\begin{answer} $\ds x^3/3+47x^2/2-5x+k$
\end{answer}

\iflatetranscendentals\else
\exercise Describe all functions with derivative $\ds {1\over 1+x^2}$.
\begin{answer} $\arctan x + k$
\end{answer}

\exercise Describe all functions with derivative $\ds x^3-{1\over x}$.
\begin{answer} $\ds x^4/4 -\ln x +k$
\end{answer}
\fi

\exercise Describe all functions with derivative $\sin(2x)$.
\begin{answer} $-\cos(2x)/2 +k$
\end{answer}

\exercise Show that the equation $\ds 6x^4 -7x+1 =0$ does not have more
than two distinct real roots.

\exercise Let $f$ be differentiable on $\R$. Suppose that $f'(x) \neq
0$ for every $x$. Prove that $f$ has at most one real root.
 
\exercise Prove that for all real $x$ and $y$
$|\cos x -\cos y | \leq |x-y|$.
