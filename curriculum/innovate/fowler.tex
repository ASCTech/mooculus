...
\draw[fill=black,opacity=0.5] (current page.north west) rectangle (current page.south east);
\end{tikzpicture}
}}
...
\draw[fill=white] (current page.north west) rectangle (current page.south east);
\node[draw,very thick,minimum width=\paperwidth-\pgflinewidth,minimum height=\paperheight-\pgflinewidth] [anchor=south west] (mynode) {\includegraphics[width=\paperwidth]{#1}};
\draw[fill=white,opacity=0.5] (current page.north west) rectangle (current page.south east);
\end{tikzpicture}
}}
...
% \begin{frame}
% \huge\textbf\textsf{http://go.osu.edu/mooculus}
% \end{frame}
