...
\end{tikzpicture}
\end{center}
  \caption{Parameters in the HMM; the priors must sum to one, meaning $p_k + p_u = 1$; the transition probabilities need to satisfy $t_{k,k} + t_{k,u} = t_{u,u} + t_{u,k} = 1$; the emission probabilities within each hidden state must likewise sum to one, e.g.,$k_{\mbox{\footnotesize ace}} + k_{\mbox{\footnotesize hint}} + k_{\mbox{\footnotesize correct}} + k_{\mbox{\footnotesize incorrect}}=1$.  There are, therefore, nine free parameters.}
