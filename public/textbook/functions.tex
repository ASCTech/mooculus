...
Finally let's take some time to discuss the syntax associated to
functions.

\subsection{Names and Parenthesis}


Given a function, we typically name it ``$f$'' for \textit{function}.
In this text we will consistently abuse notation and write $f(x)$ for
the function---this is to remind you that you are ``plugging''
something into this function to get a value out.
If one desires, they could give functions more elaborate names
\[
\mathrm{height(t)} \qquad \text{or even} \qquad \text{\Smiley}(x).
\]
Unfortunately, calculus texts treat some functions different than others. 
While one almosts always sees $f(x)$ written with parenthisis, it is very common to write
\[
\sin x \qquad \text{rather than} \qquad \sin(x).
\]
This can be confusing, as might not realize that 
\[
\sin 2x^3  = \sin\left(2x^3\right)
\]
but
\[
\sin 2x + 1 = \sin(2x)+1.
\]
We encourage the reader to always use parathensis, as it is less
ambiguious notation.
