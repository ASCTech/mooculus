...
&= \lim_{h\to 0} h \frac{f(a+h) - f(a)}{h} &\text{Set $x = a+h$.} \\
&= \lim_{h\to 0} h \lim_{h\to 0}\frac{f(a+h) - f(a)}{h} &\text{Limit Law.} \\
&= 0\cdot f'(a) = 0.
\end{align*}
Since 
\[
...
\begin{tikzpicture}
	\begin{axis}[
          ...
          \node at (axis cs:4,4.3) [anchor=west] {${\color{penColor5}f'(x)}+{\color{penColor4}g'(x)}$};
