...
\]
In many cases, particularly interesting ones, $x$ and $y$ will be
related in some other way, let's see a concrete example. 
...
\begin{solution}
We'll try to use a general stragety to solve this problems. To start,
...
Next we'll need to \textbf{find an equation}. 
...
to know. By the Pythagorean Theorem we know that $x^2+9=y^2$. 

Now we'll \textbf{differentate the equation}. Write
$$ 2x \dot x = 2y\dot y.$$ We are interested in the time at which
$x=4$; at this time we know that $4^2+9=y^2$, so $y=5$. So now we'll
...
\begin{enumerate}
\item Draw a picture.
\item Find an equation.
\item Differentiate the equation.
\item Evaluate the equation at the desired values. 
\end{enumerate}
...
equation $V=4\pi r^3/3$.  Taking the derivative of both sides gives
$dV/dt=4\pi r^2\dot r$.  We now substitute the values we know at the
instant in question: $7=4\pi 4^2\dot r$, so
$\dot r=7/(64\pi)$ cm/sec.
\end{example}
...
$V=\pi r^2h/3$.  We know $dV/dt$, and we want $dh/dt$.  At
...
problem entirely: $V=\pi(h/3)^2h/3=\pi h^3/27$.  We take
the derivative of both sides and plug in $h=4$ and $dV/dt=10$, obtaining
$10=(3\pi\cdot 4^2/27)(dh/dt)$.  Thus, $dh/dt=90/(16\pi)$
cm/sec.
\end{example}
...
(b) the rate we want is $\dot{\theta}=d\theta/dt$, where $\theta$ stands for
...
$x^2+(10-y)^2=100$.  Taking the derivative of both sides we obtain:
...
leg 6), and $\dot{x}=6$.  Putting in these values gives us
$2\cdot 6\cdot 6-2\cdot 8\dot{y}=0$, from which we can easily solve
for $\dot{y}$: $\dot{y}=4.5$ ft/sec.
...
obtain $(\cos\theta)\dot{\theta}=0.1\dot{x}$.  At the instant in
question ($t=1$ sec), when we have a right triangle with sides
6--8--10, $\cos\theta=8/10$ and $\dot{x}=6$. Thus
$(8/10)\dot{\theta}=6/10$, i.e., $\dot{\theta}=6/8=3/4$ rad/sec, or
...
Theorem, $c(t)^2=a(t)^2+b(t)^2$. Taking derivatives
we get $2c(t)c'(t)=2a(t)a'(t)+2b(t)b'(t)$, so
$$
