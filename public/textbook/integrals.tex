...
Given an interval $[a,b]$ and a partititon defined by
\[
a = x_0 < x_1 <x_2 < \cdots x_{n-1}< x_n = b,
\]
...
\end{itemize}
\end{definition}
Riemann sums give a mechnisim though which integrals could be
computed. Let's compute some Riemann sums.
...
\]
using four equally spaced parititons of the interval $[1,2]$.
\end{example}
...
\begin{tikzpicture}
	\begin{axis}[
        ...
        \end{axis}
\end{tikzpicture}
\caption{Here we see the interval $[1,2]$ partititoned into four
  subintervals.}
...
For a Riemann sum to equal the value of the related integral, we need
the width of our parititions to go to zero. We'll work through an
example of this.
...
\end{align*}
Now at this point we need two formulas
\[
