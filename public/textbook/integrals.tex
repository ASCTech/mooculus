...
subinterval $[x_i,x_{i+1}]$ and the height $f(x_i^*)$. Adding the
areas of these rectangles together we find
\[
...
\begin{tikzpicture}
	\begin{axis}[
        ...
        \end{axis}
\end{tikzpicture}
\caption{We'll use a sum to compute $\int_3^7 2x -1 \d x$. Note if
  there are $n$ rectangles, then each rectangle is of width $4/n$.}
...
with $n$ equally spaced partitions that approximates $\int_1^3 4-x^2 \d x$.
\begin{answer}
...
\begin{exercise}
Use a Riemann sum to compute $\int_{-1}^3 4-x \d x$.
\begin{answer}
$12$
\end{answer}
\end{exercise}
