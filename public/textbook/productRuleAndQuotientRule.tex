...
\begin{tikzpicture}
	\begin{axis}[
          ...
          \node at (axis cs:5,3.91) [anchor=west] {${\color{penColor}f(a)}{\color{penColor4}g'(a)h}+{\color{penColor5}f'(x)h}{\color{penColor2}g(x)}$};
...
\caption{A geometric interpretation of the product rule. Since every
  ...
  $f(x)$ and $g(x)$, the moving a ``small amount,'' $h$, increases $f(a)g(a)$ by the
  sum of $f(x)g'(x)h$ and $f'(x)hg(x)$.}
\end{marginfigure}
