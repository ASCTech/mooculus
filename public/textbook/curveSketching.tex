...
$x=1$. Now we can check points \textbf{between} the critical values to find
...
\begin{tikzpicture}
	\begin{axis}[
          ...
          \addplot [very thick, penColor, smooth] {3*x^2-1};
          \addplot [very thick, penColor4, smooth] {6*x};         
          \node at (axis cs:-.75,1) [anchor=west] {\color{penColor}$f(x)$};  
          ...
          \addplot[color=penColor4,fill=penColor4,only marks,mark=*] coordinates{(0,0)};  %% closed hole
        \end{axis}
\end{tikzpicture}
...
maximum at $x=-2$, and a local minimum at $x=3$, see Figure~\ref{figure:SDT(x^4)/4 + (x^3)/3 -x^2}.
\end{solution}
\begin{marginfigure}[-3in]
\begin{tikzpicture}
	\begin{axis}[
...
In this short section, we will give some general guidelines for
skeketching the plot of a function.

\begin{procedureForPlotting}\hfil
\begin{itemize}
\item Find the $y$-intercept, this is the point $(0,f(0))$. Place this
  point on your graph.
\item Find vertical asymptotes, these are points where $f(x)$ is undefined.
\item Compute $f'(x)$ and $f''(x)$.
\item Find the critical values, the points where $f'(x) = 0$. 
...
\item Find the candadates for inflection points, the points where $f''(x) = 0$.
...
\end{itemize}
At this point you should be able to skech the plot of your function.
\end{procedureForPlotting}

Let's see this procedure in action. We'll sketch the plot of
$2x^3-3x^2-12x$ on the interval $[-2,4]$.  Following our guidlines
above, we start by computing $f(0) = 0$.  Hence we see that the
$y$-intercept is $(0,0)$. Place this point on your plot:
\begin{marginfigure}[0in]
\begin{tikzpicture}
	\begin{axis}[
            domain=-2:4,
...
The critical values are where $f'(x) = 0$, thus we need to solve $6x^2
-6x -12 = 0$ for x. Write
\begin{align*}
6x^2 -6x -12 &= 0 \\
x^2 - x -2 &=0\\
...
\]
Mark the critical values $x=2$ and $x=-1$ on your plot.
\begin{marginfigure}[0in]
\begin{tikzpicture}
	\begin{axis}[
            domain=-2:4,
         ...
         \addplot [dashed, penColor2, smooth] plot coordinates {(-1,-20) (-1,30)}; 
         \addplot [dashed, penColor2, smooth] plot coordinates {(2,-20) (2,30)}; 
         \addplot[color=penColor,fill=penColor,only marks,mark=*] coordinates{(0,0)};  %% closed hole
        \end{axis}
\end{tikzpicture}
\caption{A plot of ??.}
...
The candiates for the inflection points are where $f''(x) = 0$, thus
...
add this information to our plot, see Figure~\ref{}.
...
\]
We now have a complete sketch of $f(x)$ on the interval $[-2,4]$, see Figure~\ref{}.
\begin{marginfigure}[0in]
\begin{tikzpicture}
	\begin{axis}[
            axis on top=true,
