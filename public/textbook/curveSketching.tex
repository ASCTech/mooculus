...
minimum, or neither at a point? Use the first derivative test.
...
\item If there $f'(x)$ has the same sign to the left and right of
  $f'(a)$, then $f'(a)$ is not a local extremum.
\end{itemize}
\end{mainTheorem}
...
decreasing on $(-\infty,-2)\cup(0,1)$. Moreover, from the first derivative test, Theorem~\ref{T:fdt}, the local maximum is
at $x=0$ while the local minima are at $x=-2$ and $x=1$, see
...
studying how $f'(x)$ changes. This leads us in to our next section.
...
accurate picture. It is worth summerizing what we have seen already in
to a single theorem.
...
\begin{tikzpicture}
	\begin{axis}[
          ...
          \node at (axis cs:-.7,1) [anchor=west] {\color{penColor}$f(x)$};  
          \node at (axis cs:.2,1) [anchor=west] {\color{penColor4}$f''(x)$};
        \end{axis}
\end{tikzpicture}
\caption{A plot of $f(x) = 3x^2-1$ and $f''(x) = 6x$. We can see the
  concavity change at $x=0$.}
\label{figure:3x^2-1}
\end{marginfigure}
...
If $f'(x)$ changes from positive to negative it is
decreasing; this means that the derivative of $f'(x)$, $f''(x)$, might
be negative, and if in fact $f''(x)$ is negative then $f'$ is
definitely decreasing, so there is a local maximum at the point in
question. Note well that $f'(x)$ might change from positive to
negative while $f''(x)$ is zero, in which case $f''(x)$ gives us no
information about the critical value. Similarly, if $f'(x)$ changes
from negative to positive there is a local minimum at the point, and
$f'$ is increasing. If $f''>0$ at the point, this tells us that $f'$
is increasing, and so there is a local minimum.
