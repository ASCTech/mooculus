...
Thus from Theorem~\ref{theorem:theorem:diff->cont}, we see that all
...
\end{enumerate}
\begin{answer}
(a) $[0,4.5)\cup(4.5,6]$, (b) $[0,3)\cup(3,6]$, (c) See~\begin{marginfigure}[0in]
  \begin{tikzpicture}
	\begin{axis}[
            domain=0:6,
            ymax=1.5,
...
\caption{A sketch of $f'(x)$.}
\end{marginfigure}
~Figure~\ref{figure:answer-cont-diff}
\end{answer}
\end{exercise}
...
\]
Note, by the Binomial Theorem, we write $\binom{n}{k}$ for the various
...
Write
\begin{align*}
\ddx x^5+5x^2 &= \ddx x^5 + \ddx(5x^2) \\
&=5x^4+5\ddx(x^2) \\
&= 5x^4+5\cdot 2x^1 = 5x^4+10x.
\end{align*}
\end{solution}
...
\[
\lim_{h\to 0}\frac{a^h-1}{h} \approx .7 \qquad\text{and}\qquad \lim_{h\to 0} \frac{3^h-1}{h} \approx 1.1.
\]
