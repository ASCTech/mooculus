...
\marginnote{To gain
  intuition, you should compute the derivative of $f(x) = 6$ using the
...
Reasons this result makes conceptual sense:
\begin{itemize}
\item The constant function plots a horizontal line---so the slope of
  the tangent line is $0$.
\item If a object is a constant distance away from a reference point
  (that is, it isn't moving) then it's velocity is zero.
\item If an object is traveling at a constant rate, then it's
...
time.'' The \textit{sum rule} allows us to do this.
...
Reasons this result makes conceptual sense:
\begin{itemize}
\item Viewing the derivative as the velocity of an object, the sum
  rule is something like a person walking on a moving bus. The
  velocities add.
\end{itemize}
