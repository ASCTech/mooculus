...
\begin{theorem}[Differenitability implies Continuity]\label{theorem:diff->cont}
...
\end{enumerate}
\begin{answer}
(a) $[0,4.5)\cup(4.5,6]$, (b) $[0,3)\cup(3,6]$, (c) See Figure\begin{marginfigure}[0in]
  \begin{tikzpicture}
	\begin{axis}[
            domain=0:6,
            ymax=1.5,
...
\begin{theorem}[The Sum Rule]\index{derivative rules!sum}\index{sum rule}
...
\begin{tikzpicture}
	\begin{axis}[
          ...
          \node at (axis cs:1.5,1) [anchor=west,penColor] {$f(x)$};
          \node at (axis cs:2,2.4) [anchor=west,penColor2] {$g(x)$};
          \node at (axis cs:.8,3) [anchor=west,penColor3] {$f(x)+g(x)$};

          \node at (axis cs:4,1.4) [anchor=west,penColor5] {$f'(x)$};
          \node at (axis cs:4,2.9) [anchor=west,penColor4] {$g'(x)$};
          %\node at (axis cs:4,4.3) [anchor=west] {$f'(x)+g'(x)$};
          \node at (axis cs:4.6,4.3) [anchor=west] {$+$};
          \node at (axis cs:4,4.3) [anchor=west,penColor5] {$f'(x)$};
          \node at (axis cs:4.9,4.3) [anchor=west,penColor4] {$g'(x)$};
          \node at (axis cs:3.5,1.2) [anchor=north] {$\underbrace{\hspace{.50in}}_{1}$};
        \end{axis}
\end{tikzpicture}
\label{figure:sum-rule}
\caption{A geometric interpretation of the sum rule.}
\end{marginfigure}
...
\begin{theorem}[The Derivative of $\pmb{e^x}$]\index{ex@$e^x$}\index{derivative!of ex@of $e^x$}
\[
\ddx e^x = e^x.
\]
\end{theorem}
...
situation.
\begin{theorem}[The Product Rule]\index{derivative rules!product}\index{product rule}
...
Let's return to the example we started.
\begin{example} 
