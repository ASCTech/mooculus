...
\begin{tikzpicture}
	\begin{axis}[
	  ...
	  \addplot [very thick,penColor, smooth] {-(x-2)^2+4};
        ...
        \end{axis}
\end{tikzpicture}
\caption{A geometric interperation of Rolle's Theorem.}
...
\begin{tikzpicture}
	\begin{axis}[
	  ...
	  \addplot [very thick,penColor, smooth,domain=(1:5)] {sin(deg(x))+1.5};
        ...
        \end{axis}
\end{tikzpicture}
\caption{A geometric interpertation of the Mean Value Theorem}
\label{figure:geoMVT}
\end{marginfigure}
\begin{proof}
Let 
...
the starting time with $1/2$ hourss being the final time, the Mean Value Theorem states there is a time $c$
\[
...
whose dervative is zero on an inteval that is not constant? As we will
...
$f(b)=f(a)$. Moreover, since $a$ and $b$ were arbitrarly choosen,
